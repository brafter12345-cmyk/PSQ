...
const changeLogRows = [
  ...
  [
    "2026-05-16", "SCN-027",
    "Industry-aware records-per-revenue divisor and Data Breach Model Assumption Notice disclosure. The previous flat divisor (R50,000 per record) materially understated breach cost for record-heavy industries (finance, healthcare, telecoms) and overstated for B2B industries (manufacturing, construction, agriculture). New per-industry divisor table in \\_calculate\\_zar: finance R5-7.5k, healthcare R9k, telecoms R5k, retail R30k, insurance R18k, education R25k, manufacturing R500k, construction R500k, agriculture R1M, etc. Drives estimated\\_records and the breach component (records x cost\\_per\\_record). New 'Data Breach Model Assumption Notice' block in PDF + HTML explicitly surfaces the heuristic and an outlier threshold (2x estimate) above which the breach cost is flagged as 'materially understated' - directs the broker to verify record holdings with the insured and request a recalibrated estimate if appropriate.",
    "User-flagged 2026-05-16: data breach cost is currently driven by (records x cost\\_per\\_record) where records = revenue / R50k flat. This is right on average but wrong for outliers - a small fintech with 500k user records on R10M revenue would land at 200 records, understating the breach component by ~2500x. The new sliding-scale divisor better matches industry reality on average; the outlier disclosure handles the residual cases (small fintechs, health-tech aggregators, marketing platforms, breach-list resellers) where revenue is a poor proxy for record holdings. FAIS appropriate-disclosure compliance: the model's assumption must be visible to the broker / client so they can flag when the calculation does not match reality.",
    "Done 2026-05-16"
  ]
];
